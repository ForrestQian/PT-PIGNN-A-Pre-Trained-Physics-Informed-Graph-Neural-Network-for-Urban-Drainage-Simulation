...
def model_couple_train(model, train_loader, test_loader, test_node_indices, test_edge_indices,
                       train_node_indices, train_edge_indices, full_node_matrix,
                     full_edge_matrix, link_diameter, diameter_matrix, roughness_matrix, offset_up_matrix, offset_down_matrix,
                     max_lists, min_lists, link_length_list, Criterion_mass, Criterion_momentum, Criterion_node, Criterion_edge, Optimizer, viz=None, epochs=1):
    model.train()
    ...
    df_loss = pd.DataFrame(columns=['Epoch', 'Batch', 'PDE_Mass_Loss', 'PDE_Momentum_Loss', 'Total_PDE_Loss',
                                    'Node_Loss', 'Edge_Loss', 'Total_Data_Loss', 'Total_Loss'])
    ...
    for epoch in tqdm(range(epochs), desc="Training Epochs"):
        ...
        for batch in tqdm(train_loader, desc="Training Batches", position=0):
            if draw_time % check_interval == 1:
                target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse = model_test(
                    model, test_loader, test_node_indices,
            ...
            PDE_1, PDE_2 = PDE_Loss(node_inflow, expanded_e, n_index=roughness_matrix, D_index=diameter_matrix,
                                    g=9.81, S0_index=offset_down_matrix - offset_up_matrix,
                                    length_index=link_length_list)
            loss_mass = Criterion_mass(PDE_1, torch.zeros_like(PDE_1))
            loss_momentum = Criterion_momentum(PDE_2, torch.zeros_like(PDE_2))
            loss_pde = loss_mass
            loss_data = lambda3 * loss_node + (1 - lambda3) * loss_edge
            loss = loss_pde * lambda1 + loss_data * (1 - lambda1)
            loss.backward()
            Optimizer.step()
            Optimizer.zero_grad()
            end_time = time.time()
            ...
            new_loss_row = pd.DataFrame({'Epoch': [epoch + 1],
                                         'Batch': [draw_time],
                                         'PDE_Mass_Loss': [loss_mass.item()],
                                         'PDE_Momentum_Loss': [loss_momentum.item()],
            ...
            if viz is not None:
                ...
                viz.line([loss.item()], [draw_time], win='Data_Loss', name='Total_Data_Loss',
                         update='append', opts=dict(title='Data_Loss'))
                viz.line([loss_mass.item()], [draw_time], win='PDE_Loss', name='Mass_Loss',
                         update='append', opts=dict(title='PDE_Loss'))
                viz.line([loss_momentum.item()], [draw_time], win='PDE_Loss', name='Momentum_Loss',
                         update='append', opts=dict(title='PDE_Loss'))
                viz.line([loss_pde.item()], [draw_time], win='PDE_Loss', name='Total_PDE_Loss',
                         update='append', opts=dict(title='PDE_Loss'))
...
if __name__ == "__main__":
    ...
    viz = Visdom(env='Changzhou_PIGNN-Mass')
    ################## Dataset preparation ##################
    ...
    model = model_couple_train(model, train_data_loader, test_data_loader, test_node_indices, test_edge_indices,
                               train_node_indices, train_edge_indices,
                                    ...
                                    Criterion_mass, Criterion_momentum, Criterion_node, Criterion_edge, Data_Optimizer, viz, epochs=100)
