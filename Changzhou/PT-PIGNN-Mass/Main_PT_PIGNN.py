...
import sys
import torch
from PIGNN_Net import GCN_Model
...
def model_test(model, test_loader, node_indices, edge_indices, link_diameter, diameter_matrix, full_node_matrix, full_edge_matrix, need_plot=False, viz=None, draw_time=1):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc="Testing Batches", position=0):
            ...
            h_out, edge_out, _, _, _, _ = model(node_input, edge_input, link_diameter, diameter_matrix, full_node_matrix, full_edge_matrix)
...
def model_data_train(model, train_loader, test_loader, train_node_indices, train_edge_indices,
                     test_node_indices, test_edge_indices, link_diameter, diameter_matrix, full_node_matrix,
    ...
    check_interval = 8 * 27  # Run evaluation and checkpoint every 16 training batches
        ...
        for batch in tqdm(train_loader, desc="Training Batches", position=0):
            if draw_time % check_interval == 1:
                target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse = model_test(model, test_loader, test_node_indices,
                                                                                    test_edge_indices, link_diameter, diameter_matrix,
            ...
            node_target = node_target.reshape((node_target.shape[0]), -1)
            edge_target = edge_target.reshape((edge_target.shape[0]), -1, edge_target.shape[-1])
            h_out, edge_out, expanded_e, lambda1, lambda2, lambda3 = model(node_input, edge_input, link_diameter, diameter_matrix, full_node_matrix, full_edge_matrix)
            ...
            draw_time += 1
            # Clear redundant GPU memory and RAM
            del node_input, edge_input, node_target, edge_target, h_out, edge_out, expanded_e
...
def model_physical_pretrain(model, train_loader, test_loader, test_node_indices, test_edge_indices, full_node_matrix,
                     full_edge_matrix, link_diameter, diameter_matrix, roughness_matrix, offset_up_matrix, offset_down_matrix,
                     max_lists, min_lists, link_length_list, Criterion_mass, Criterion_momentum, Optimizer, viz=None, pde_batch=1, epochs=1):
    model.train()
    check_interval = 16 # Run evaluation and checkpoint every 16 training batches
    df_loss = pd.DataFrame(columns=['Epoch', 'Mass_Loss', 'Momentum_Loss', 'Total_PDE_Loss'])
    df_time = pd.DataFrame(columns=['Epoch', 'Time_Seconds'])
    node_columns = [f'Node_{i}' for i in test_node_indices]
    flow_columns = [f'Edge_{i}_Flow' for i in test_edge_indices]
    velocity_columns = [f'Edge_{i}_Velocity' for i in test_edge_indices]
    df_node_nse = pd.DataFrame(columns=node_columns)
    df_edge_flow_nse = pd.DataFrame(columns=flow_columns)
    df_edge_velocity_nse = pd.DataFrame(columns=velocity_columns)
    df_lambda = pd.DataFrame(columns=['Epoch', 'Lambda1', 'Lambda2', 'Lambda3'])
    ...
    for batch in train_loader:
        ...
        node_input_shape = node_input.shape
        # Drop batch dim and record input shape
        node_input_shape = node_input_shape[1:]
        edge_input_shape = edge_input.shape
        edge_input_shape = edge_input_shape[1:]
        # Add PDF batch dimension
        node_input_shape = (pde_batch,) + node_input_shape
        edge_input_shape = (pde_batch,) + edge_input_shape
        break

    for epoch in tqdm(range(epochs), desc="Physical Pretraining Epochs"):
        if epoch % check_interval == 0:
            target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse = model_test(model, test_loader, test_node_indices,
                                                                                test_edge_indices, link_diameter, full_diameter_matrix,
                                                                                full_node_matrix, full_edge_matrix,
                                                                                need_plot=True, viz=viz, draw_time=epoch)
            new_node_nse = pd.DataFrame([node_nse], columns=node_columns)
            new_edge_flow_nse = pd.DataFrame([flow_nse], columns=flow_columns)
            new_edge_velocity_nse = pd.DataFrame([velocity_nse], columns=velocity_columns)
            if len(df_node_nse) == 0:
            ...
            df_lambda.to_csv('lambda_values.csv', index=False)
            torch.save(model.state_dict(),
                       f'checkpoint_physical_model_epoch{epoch + 1}.pth')
            # Clear redundant GPU memory and RAM
            del target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse
            del new_node_nse, new_edge_flow_nse, new_edge_velocity_nse
            if torch.cuda.is_available():
            ...
            gc.collect()
        start_time = time.time()
        node_input = torch.randn(node_input_shape)
        node_input[:, :, 0] = node_input[:, :, 0] * (max_lists[0] - min_lists[0]) + min_lists[0]  # Randomly initialize water level
        node_input[:, :, 1] = node_input[:, :, 1] * (max_lists[1] - min_lists[1]) + min_lists[1]  # Randomly initialize inflow
        # Stack inflow (node_input[:, :, 1]) 300 times along the third dimension
        node_inflow = node_input[:, :, 1].unsqueeze(dim=2).repeat(1, 1, 300).to(device)
        edge_input = torch.randn(edge_input_shape)
        edge_input[:, :, 0] = edge_input[:, :, 0] * (max_lists[2] - min_lists[2]) + min_lists[2]  # Randomly initialize flow
        edge_input[:, :, 1] = edge_input[:, :, 1] * (max_lists[3] - min_lists[3]) + min_lists[3]  # Randomly initialize velocity
        node_input = node_input.to(device)
        edge_input = edge_input.to(device)
        _, _, expanded_e, lambda1, lambda2, lambda3 = model(node_input, edge_input, link_diameter, full_diameter_matrix, full_node_matrix, full_edge_matrix)
        PDE_1, PDE_2 = PDE_Loss(node_inflow, expanded_e, n_index=roughness_matrix, D_index=diameter_matrix,
                               g=9.81, S0_index=offset_down_matrix - offset_up_matrix, length_index=link_length_list)
        loss_mass = Criterion_mass(PDE_1, torch.zeros_like(PDE_1))
        loss_momentum = Criterion_momentum(PDE_2, torch.zeros_like(PDE_2))
        loss_pde = loss_mass
        end_time = time.time()
        elapsed_time = end_time - start_time
        new_loss_row = pd.DataFrame({'Epoch': [epoch + 1],
                                     'Mass_Loss': [loss_mass.item()],
                                     'Momentum_Loss': [loss_momentum.item()],
        ...
        Optimizer.zero_grad()
        if viz is not None:
            viz.line([loss_mass.item()], [epoch + 1], win='PDE_Loss', name='Mass_Loss',
                     update='append', opts=dict(title='PDE_Loss'))
            viz.line([loss_momentum.item()], [epoch + 1], win='PDE_Loss', name='Momentum_Loss',
                     update='append', opts=dict(title='PDE_Loss'))
            viz.line([loss_pde.item()], [epoch + 1], win='PDE_Loss', name='Total_PDE_Loss',
                     update='append', opts=dict(title='PDE_Loss'))
            viz.line([lambda1.item()], [epoch + 1], win='Lambda_Values', name='Lambda1',
                     update='append', opts=dict(title='Lambda_Values'))
            viz.line([lambda2.item()], [epoch + 1], win='Lambda_Values', name='Lambda2',
                     update='append', opts=dict(title='Lambda_Values'))
            viz.line([lambda3.item()], [epoch + 1], win='Lambda_Values', name='Lambda3',
                     update='append', opts=dict(title='Lambda_Values'))
        # Clear redundant GPU memory and RAM
        del node_input, edge_input, node_inflow, expanded_e, PDE_1, PDE_2
        del loss_mass, loss_momentum, loss_pde, new_loss_row, new_time_row, new_lambda_row
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
    return model
...
if __name__ == "__main__":
    ...
    viz_data = Visdom(env='Changzhou_PT-PIGNN-Mass_Data-FineTune')
    viz_physical = Visdom(env='Changzhou_PT-PIGNN-Mass_Physical-Pretrain')
    ################## Dataset preparation ##################
    ...
    (edge_index, train_data_loader, test_data_loader, sensor_node_indices, sensor_link_indices, mask_node_indices, mask_link_indices,
     ...
     node_area_list, node_pos, max_lists, min_lists) = dataset_create(train_batch_size=4, test_batch_size=8, force_recreate=True)
    ...
    ############# Model instantiation #############
    model = GCN_Model(resolution=resolution, hops_node=hopes_node, hops_edge=hopes_edge,
    ...
    Criterion_mass = torch.nn.L1Loss()
    Criterion_momentum = torch.nn.L1Loss()
    ...
    PDE_Optimizer = torch.optim.Adam(model.parameters(), lr=lr_pde, weight_decay=weight_decay_pde)
    print("Optimizer created successfully.")

    #############  Physical pre-training #############
    model = model_physical_pretrain(model, train_data_loader, test_data_loader, test_node_indices, test_edge_indices,
                                    full_node_matrix, full_edge_matrix, diameter_matrix, full_diameter_matrix, full_roughness_matrix,
                                    full_offset_up_matrix, full_offset_down_matrix, max_lists, min_lists, link_length_list,
                                    Criterion_mass, Criterion_momentum, PDE_Optimizer, viz_physical, pde_batch=8, epochs=1000)
    # Load trained model weights
    model.load_state_dict(torch.load(''))
    ############## Data fine-tuning #############
    model = model_data_train(model, train_data_loader, test_data_loader, train_node_indices, train_edge_indices,
                             test_node_indices, test_edge_indices, diameter_matrix, full_diameter_matrix, full_node_matrix, full_edge_matrix,
                             Criterion_node, Criterion_edge, Data_Optimizer, viz_data, epochs=100)

    # ############## Model testing #############
    model.load_state_dict(torch.load('', map_location=device))
    target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse = model_test(model,
                                                                                                          test_data_loader,
                                                                                                          test_node_indices,
                                                                                                          test_edge_indices,
                                                                                                          diameter_matrix,
                                                                                                          full_diameter_matrix,
                                                                                                          full_node_matrix,
                                                                                                          full_edge_matrix,
                                                                                                          need_plot=True,
                                                                                                          viz=viz_data,
