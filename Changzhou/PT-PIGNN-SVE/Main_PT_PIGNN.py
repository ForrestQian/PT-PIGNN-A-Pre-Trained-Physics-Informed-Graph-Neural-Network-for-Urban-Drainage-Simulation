...
def model_test(model, test_loader, node_indices, edge_indices, diameter_matrix, full_node_matrix, full_edge_matrix, need_plot=False, viz=None, draw_time=1):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc="Testing Batches", position=0):
            ...
            h_out, edge_out, _, _, _, _ = model(node_input, edge_input, diameter_matrix, full_node_matrix, full_edge_matrix)
...
def model_data_train(model, train_loader, test_loader, train_node_indices, train_edge_indices,
    ...
    for epoch in tqdm(range(epochs), desc="Training Epochs"):
        ...
        for batch in tqdm(train_loader, desc="Training Batches", position=0):
            ...
            node_target = node_target.reshape((node_target.shape[0]), -1)
            edge_target = edge_target.reshape((edge_target.shape[0]), -1, edge_target.shape[-1])
            h_out, edge_out, expanded_e, lambda1, lambda2, lambda3 = model(node_input, edge_input, diameter_matrix, full_node_matrix, full_edge_matrix)
...
def model_physical_pretrain(model, train_loader, test_loader, test_node_indices, test_edge_indices, full_node_matrix,
    ...
    for epoch in tqdm(range(epochs), desc="Physical Pretraining Epochs"):
        if epoch % check_interval == 0:
            target_nodes, output_nodes, target_edges, output_edges, node_nse, flow_nse, velocity_nse = model_test(model, test_loader, test_node_indices,
                                                                                test_edge_indices, link_diameter,
                                                                                full_node_matrix, full_edge_matrix,
        ...
        _, _, expanded_e, lambda1, lambda2, lambda3 = model(node_input, edge_input, link_diameter, full_node_matrix, full_edge_matrix)
        ...
        loss_pde = lambda2 * loss_mass + (1 - lambda2) * loss_momentum
        end_time = time.time()
        elapsed_time = end_time - start_time
        new_loss_row = pd.DataFrame({'Epoch': [epoch + 1],
...
if __name__ == "__main__":
    ...
    viz_data = Visdom(env='Changzhou_PT-PIGNN-SVE_Data-FineTune')
    viz_physical = Visdom(env='Changzhou_PT-PIGNN-SVE_Physical-Pretrain')
    ################## Dataset preparation ##################
