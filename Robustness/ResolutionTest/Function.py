...
def PDE_Loss(q, expanded_e, n_index, D_index, g, S0_index, length_index, resolution):
    q = q[:, :, :] * 100 / 3600
    w = expanded_e[:, :, :, 0]
    u = expanded_e[:, :, :, 1]
    ...
    flow = s * u
    dAdt = DT_Get(s)
    dQdx = DX_Get(flow) / resolution
    dudt = DT_Get(u)
    dudx = DX_Get(u) / resolution
    dydx = DX_Get(y) / resolution
