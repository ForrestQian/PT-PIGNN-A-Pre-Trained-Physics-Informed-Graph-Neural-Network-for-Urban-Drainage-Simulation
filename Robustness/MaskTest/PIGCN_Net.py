...
from Function import inverse_f, Area_np, Area
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# GCN dataset
class GCN_Dataset(Dataset):
    def __init__(self, edge_index, length_index, file_path):

        self.file_path = file_path
        self.length_index = length_index
        self.max_length = torch.max(length_index)
    ...
    def __getitem__(self, index):
        ...
        node_feature_h = self.node_data_h[index, :].astype(np.float32)
        node_feature_h = torch.tensor(node_feature_h).unsqueeze(dim=1)

        node_feature_q = self.node_data_q[index, :].astype(np.float32)
        ...
        edge_input_capacity = self.edge_data_capacity[index, :].astype(np.float32)
        edge_input_capacity = torch.tensor(edge_input_capacity)

        edge_input_velocity = self.edge_data_velocity[index, :].astype(np.float32)
        ...
        edge_feature = torch.concatenate((edge_input_capacity.unsqueeze(dim=1), edge_input_velocity.unsqueeze(dim=1)), dim=1)

        output_node_feature = self.node_data_h[index + 1, :].astype(np.float32)
        output_node_feature = torch.tensor(output_node_feature)

        output_edge_capacity = self.edge_data_capacity[index + 1, :].astype(np.float32)
        output_edge_capacity = torch.tensor(output_edge_capacity).unsqueeze(dim=1)

        output_edge_velocity = self.edge_data_velocity[index + 1, :].astype(np.float32)
...
class GCN_Model(nn.Module):
    ...
    def __init__(self, resolution, hops_node, hops_edge,
        ...
        base_length = torch.as_tensor(length_index, dtype=torch.float32)
        # resolution: edge spatial discretization step (m); 1 = per meter, 0.1 = refine to 0.1 m
        refined_length = torch.round(base_length / float(resolution)).to(torch.long)
        refined_length = torch.clamp(refined_length, min=2)
        self.length_index = refined_length.tolist()
        total_refined_length = int(refined_length.sum().item())
        self.num_nodes = len(node_index)
        ...
        if transcov_hidden is not None:
            ...
            self.flow_full.append(nn.Linear(pre_neurons, total_refined_length))
            self.velocity_full.append(nn.Linear(pre_neurons, total_refined_length))
        else:
            self.flow_full.append(nn.Linear(self.num_edges, total_refined_length))
            self.velocity_full.append(nn.Linear(self.num_edges, total_refined_length))
    ...
    def forward(self, h, e, D):
        ...
        h_full = torch.concatenate((h[:, :, 0].unsqueeze(dim=2), h_updated), dim=2)
        flow_full = torch.concatenate((origin_flow.unsqueeze(dim=2), flow_updated), dim=2)
        velocity_full = torch.concatenate((origin_velocity.unsqueeze(dim=2), velocity_updated), dim=2)
        curr_index = 0

        for i in range(len(self.length_index)):
            flow_full[:, curr_index, :] = h_full[:, i, :] / D[i]
            flow_full[:, curr_index + self.length_index[i] - 1, :] = h_full[:, i + 1, :] / D[i]
            velocity_full[:, curr_index, :] = 0
            velocity_full[:, curr_index + self.length_index[i] - 1, :] = 0
        ...
        for i in range(self.num_nodes):
            if i == 0:
                ...
                h_out[:, i] = (expanded_e[:, curr_index, -1, 0] * D[i] + expanded_e[:, curr_index - 1, -1, 0] * D[i - 1]) / 2
                curr_index = curr_index + self.length_index[i]
